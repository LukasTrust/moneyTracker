...
class SummaryResponse(BaseModel):
    ...
    total_expenses: Decimal = Field(..., description="Total expenses (negative)")
    current_balance: Decimal = Field(..., description="Current balance (initial balances + transactions up to effective date)")
...
class ChartDataResponse(BaseModel):
    ...
    income: List[Decimal] = Field(..., description="Income per period")
    expenses: List[Decimal] = Field(..., description="Expenses per period")
    balance: List[Decimal] = Field(..., description="Cumulative balance per period")
...
class BalanceHistoryResponse(BaseModel):
    ...
    income: List[Decimal] = Field(..., description="Income per period")
    expenses: List[Decimal] = Field(..., description="Expenses per period")
    balance: List[Decimal] = Field(..., description="Cumulative balance")
...
class PeriodComparisonData(BaseModel):
    ...
    current_balance: Decimal = Field(..., description="Current balance for period (initial balances + transactions up to effective date)")
    transaction_count: int = Field(..., description="Number of transactions")
...
class ComparisonMetrics(BaseModel):
    ...
    income_diff: Decimal = Field(..., description="Income difference (period2 - period1)")
    income_diff_percent: float = Field(..., description="Income percentage change")
    expenses_diff: Decimal = Field(..., description="Expenses difference (period2 - period1)")
    expenses_diff_percent: float = Field(..., description="Expenses percentage change")
    balance_diff: Decimal = Field(..., description="Balance difference")
