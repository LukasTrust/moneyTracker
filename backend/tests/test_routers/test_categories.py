...
@pytest.mark.asyncio
async def test_recategorize_transactions():
    """Test recategorize_transactions endpoint - basic functionality"""
    from app.routers.categories import recategorize_transactions
    
    # Mock DB session
    mock_db = MagicMock()
    
    # Mock empty result (no transactions)
    mock_data_query = MagicMock()
    mock_data_query.filter.return_value.all.return_value = []
    mock_db.query.side_effect = lambda cls: mock_data_query if cls.__name__ == 'DataRow' else MagicMock()
    
    result = recategorize_transactions(None, mock_db)
...
@pytest.mark.asyncio
async def test_recategorize_transactions_with_account_filter():
    """Test recategorize_transactions with account filter"""
    from app.routers.categories import recategorize_transactions
    
    # Mock DB session
    mock_db = MagicMock()
    
    # Mock empty result with account filter
    mock_data_query = MagicMock()
    mock_filtered_query = MagicMock()
    mock_filtered_query.all.return_value = []
    mock_data_query.filter.return_value = mock_filtered_query
    mock_db.query.side_effect = lambda cls: mock_data_query if cls.__name__ == 'DataRow' else MagicMock()
    
    result = recategorize_transactions(1, mock_db)
    
    # Verify account filter was applied
    mock_data_query.filter.assert_called_once()
    assert result['total_transactions'] == 0


@pytest.mark.asyncio
async def test_recategorize_transactions_no_transactions():
    """Test recategorize_transactions with no transactions"""
    from app.routers.categories import recategorize_transactions
    
    # Mock DB session
    mock_db = MagicMock()
    
    # Mock DataRow query returning empty
    mock_data_query = MagicMock()
    mock_data_query.filter.return_value.all.return_value = []
    mock_db.query.side_effect = lambda cls: mock_data_query if cls.__name__ == 'DataRow' else MagicMock()
...
@pytest.mark.asyncio
async def test_recategorize_transactions_with_changes():
    """Test recategorize_transactions with transactions that get recategorized"""
    from app.routers.categories import recategorize_transactions
    ...
    mock_transaction.category_id = None  # Initially uncategorized
    mock_transaction.recipient = "Test Recipient"
    mock_transaction.purpose = "Test Purpose"
    
    # Mock category
    mock_category = MagicMock()
    mock_category.id = 2
    mock_category.name = "Matched Category"
    
    # Mock CategoryMatcher
    with patch('app.routers.categories.CategoryMatcher') as mock_matcher_class:
        mock_matcher = MagicMock()
        mock_matcher.match_category.return_value = 2  # Returns new category ID
        mock_matcher_class.return_value = mock_matcher
        
        # Mock DataRow query
        mock_data_query = MagicMock()
        mock_data_query.all.return_value = [mock_transaction]
        mock_db.query.return_value = mock_data_query
        
        # Mock Category query for distribution
        mock_category_query = MagicMock()
        mock_category_query.filter.return_value.first.return_value = mock_category
        
        # Make query return different mocks based on call
        mock_db.query.side_effect = [mock_data_query, mock_category_query]
        
        result = recategorize_transactions(None, mock_db)
        
        # Verify transaction was updated
        assert mock_transaction.category_id == 2
        ...
        assert result["category_distribution"] == {"Matched Category": 1}


@pytest.mark.asyncio
async def test_recategorize_transactions_with_uncategorized():
    """Test recategorize_transactions when matcher returns None (uncategorized)"""
    from app.routers.categories import recategorize_transactions
    ...
    with patch('app.routers.categories.CategoryMatcher') as mock_matcher_class:
        mock_matcher = MagicMock()
        mock_matcher.match_category.return_value = None  # No match found
        mock_matcher_class.return_value = mock_matcher
